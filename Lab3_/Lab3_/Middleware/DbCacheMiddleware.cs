...
namespace Lab3_.Middleware
{
    ...
    public class DbCacheMiddleware
    {
        ...
        public DbCacheMiddleware(RequestDelegate next, IMemoryCache memoryCache, string cacheKey = "Transport20")
        {
            _next = next;
        ...
        public Task Invoke(HttpContext httpContext, ITransportService transportService)
        {
            if (!_memoryCache.TryGetValue(_cacheKey, out HomeViewModel homeViewModel))
            {
                homeViewModel = transportService.GetHomeViewModel("Transport20");
